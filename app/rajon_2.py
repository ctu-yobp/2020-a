...
class Rajon(QtWidgets.QDialog,Ui_Dialog):
    ...
    def vypocet_rajonu(self):
        ...
        # vezme cestu k databazi
        self.cesta_databaze = path.ziskej_cestu()
        ...
        self.stanovisko_data = Databaze.sql_query(self.cesta_databaze[0],query_stanovisko)
        self.orientace_data = Databaze.sql_query(self.cesta_databaze[0],query_orientace)
        self.bod_data = Databaze.sql_query(self.cesta_databaze[0],query_bod)
        self.mereni_orientace = Databaze.sql_query(self.cesta_databaze[0],query_mereni_orientace)

        print(self.bod_data)

        # priprava slovniku pro vypocet rajonu
        b_ori={5001:{'x': self.orientace_data[0][3], 'y': self.orientace_data[0][2], 'smer': self.mereni_orientace[0][5]*math.pi/200}}
        b_sta={'X': self.stanovisko_data[0][3], 'Y': self.stanovisko_data[0][2]}
        b_mer={5003:{'delka': self.bod_data[0][3]*math.sin(self.bod_data[0][4]*math.pi/200), 'smer': self.bod_data[0][5]*math.pi/200}}
    ...
    def ulozeni_rajonu(self):
        ...
        print(query)
        Databaze.pridani_bodu(self.cesta_databaze[0], query)
